...
%
%    MJ:
%    will also output indices for fixations, blinks, and saccades (as interpreted by Eyelink algorithm)
%
function e = getTaskEyeTraces_m(stimfileName,varargin)
...
e.eye.samplerate = edf.samplerate;
...
% MJ edit: initialize fields
e.eye.fixations = nan(edf.nTrials,ceil(maxTrialLen/timeBetweenSamples));
e.eye.blinks = nan(edf.nTrials,ceil(maxTrialLen/timeBetweenSamples));
e.eye.saccades = nan(edf.nTrials,ceil(maxTrialLen/timeBetweenSamples));

% put in time in seconds
e.eye.time = (0:(size(e.eye.xPos,2)-1))/edf.samplerate;

% MJ edit: create index vectors for fixations, blinks, and saccades
mj.fixations = zeros(1,numel(edf.gaze.time));
mj.blinks = zeros(1,numel(edf.gaze.time));
mj.saccades = zeros(1,numel(edf.gaze.time));
for f = 1:length(edf.fixations.startTime)
   mj.fixations(edf.gaze.time>=edf.fixations.startTime(f) & edf.gaze.time<=edf.fixations.endTime(f)) = 1;
end
for b = 1:length(edf.blinks.startTime)
   % for blinks, only the start time will be indicated. users can use arbitrary blink windows to remove blink intervals
   mj.blinks(edf.gaze.time==edf.blinks.startTime(b)) = 1;
end
for s = 1:length(edf.saccades.startTime)
   mj.saccades(edf.gaze.time>=edf.saccades.startTime(s) & edf.gaze.time<=edf.saccades.endTime(s)) = 1;
end
...
for iTrial = 1:edf.nTrials
  ...
  % MJ edit: populate fields for fixations, blinks, and saccades
  fixIdx = mj.fixations(edf.gaze.time>=thisTrialTimes(1) & edf.gaze.time<=thisTrialTimes(end));
  e.eye.fixations(iTrial,1:min(length(fixIdx),length(thisTrialTimes))) = logical(fixIdx);
  blinkIdx = mj.blinks(edf.gaze.time>=thisTrialTimes(1) & edf.gaze.time<=thisTrialTimes(end));
  e.eye.blinks(iTrial,1:min(length(blinkIdx),length(thisTrialTimes))) = logical(blinkIdx);
  saccIdx = mj.saccades(edf.gaze.time>=thisTrialTimes(1) & edf.gaze.time<=thisTrialTimes(end));
  e.eye.saccades(iTrial,1:min(length(saccIdx),length(thisTrialTimes))) = logical(saccIdx);
end
