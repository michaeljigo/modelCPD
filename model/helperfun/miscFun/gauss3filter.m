...
function result=gauss3filter(image, sigma, pixelspacing)
    ...
    [u,v,w,x] = ifftshiftedcoormatrix3(size(image) );
...
% are thus authomatically moved to GPU if "image" is an GPU array.
    ...
    x=image*0 + x/size(image,4)/pixelspacing(4);

% Original Gaussian kernel    
    fil = GaussianKernel(u, v, w,x, sigma1, sigma2, sigma3,sigma4);
...
function output=GaussianKernel(u, v, w,x, sigma1, sigma2, sigma3,sigma4)
    output = exp(-2*pi*pi* (u.^2 *sigma1 + v.^2 *sigma2 + w.^2 * sigma3 + x.^2*sigma4));
end
