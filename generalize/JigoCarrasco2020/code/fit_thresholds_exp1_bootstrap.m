% Purpose:  Fit image-computable Attention model (imAmodel) to bootstrapped contrast thresholds from Experiment 1 in Jigo & Carrasco, 2020.
%
% By:       Michael Jigo
%           05.25.21

function fit_thresholds_exp1_bootstrap(varargin)
...
%% Load experimental data
   ...
   data.crf.thresh_boot = data.crf.thresh_boot(:,:,freq_idx,:);
...
%% Optimize bootstrap samples
parfor b = 1:p.nboot
   optimize_and_save(stimdrive,supdrive,attn,data,grating,energy,p,model_params,plaus_bnd,b);
end
   

function optimize_and_save(stimdrive,supdrive,attn,data,grating,energy,p,model_params,plaus_bnd,b)
   ...
   data.crf.thresh = data.crf.thresh_boot(b,:,:,:);

   % run optimization
   objective = @(params)fit_thresholds_objective_exp1_bootstrap(stimdrive,supdrive,attn,data,grating,energy,p,params);
   ...
   %% save
   savedir = '../data/fitted_parameters/bootstrap/';
   if ~exist(savedir,'dir')
      mkdir(savedir);
   end
   filename = sprintf('exp1_%s_%02d.mat',p.attn_type,b);
